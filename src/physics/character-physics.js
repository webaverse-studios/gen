...
export const avatarHmdToCapsule = (() => {
  const localVector = new THREE.Vector3();
  return (avatar, target) => {
    return target.copy(avatar.inputs.hmd.position)
...
class CharacterPhysics {
  ...
  loadCharacterController(characterWidth, characterHeight) {
    ...
    const position = avatarHmdToCapsule(this.avatar, localVector);
  ...
  applyGravity(nowS, timeDiffS) {
    const fallLoopAction = this.actionManager.getAction('fallLoop');
    if (fallLoopAction) {
      const physicsScene = physicsManager.getScene();
  ...
  applyCharacterPhysicsDetail(velocityAvatarDirection, updateRig, now, timeDiffS) {
    ...
    if (this.avatar) {
      ...
      // aesthetic jump
      const jumpAction = this.actionManager.getAction('jump');
      if (jumpAction?.trigger === 'jump') {
        const doubleJumpAction = this.actionManager.getAction('doubleJump');
        if (doubleJumpAction) {
          const doubleJumpTime =
            physx.physxWorker.getActionInterpolantAnimationAvatar(this.avatar.animationAvatarPtr, 'doubleJump', 0);
          localVector3.y =
          ...
          if (doubleJumpTime >= flatGroundJumpAirTime) {
            this.actionManager.setControlAction({type: 'fallLoop', from: 'jump'});
          }
        } else {
          const jumpTime = physx.physxWorker.getActionInterpolantAnimationAvatar(this.avatar.animationAvatarPtr, 'jump', 0);
          localVector3.y =
      ...
      if (
        this.actionManager.hasAction('swim') &&
        this.actionManager.getAction('swim').onSurface &&
        !this.actionManager.hasAction('fly')
      ) {
      ...
      if (
        !grounded &&
        !this.actionManager.getAction('jump') &&
        !this.actionManager.getAction('fly') &&
        !this.actionManager.hasAction('swim')
      ) {
      ...
      //   localVector2
      // );
      // localQuaternion.copy(this.characterController.quaternion);
      ...
      /* // capsule physics
      if (!this.actionManager.hasAction('sit')) {
        ...
        if (grounded) {
          ...
          if (!this.lastGrounded) {
            if (
              this.actionManager.hasAction('jump') ||
              this.actionManager.hasAction('fallLoop')
            ) {
              this.actionManager.setControlAction({
                type: 'land',
                time: now,
                isMoving: this.avatar.idleWalkFactor > 0,
              });
              this.actionManager.removeAction('doubleJump');
            }
          }
        } else {
          ...
          if (lastGroundedTimeDiff > 200) {
            if (
              ...
              this.actionManager.setControlAction({type: 'fallLoop'});
              this.velocity.y = 0;
            }
          }
        }
  ...
  applyVelocityDamping(velocity, timeDiffS) {
    ...
    if (this.actionManager.hasAction('fly')) {
      doDamping(flyFriction);
    } else if(this.actionManager.hasAction('swim')){
  ...
  applyCharacterPhysics(now, timeDiffS) {
    if (
      this.actionManager.hasAction('firstperson') ||
      (this.actionManager.hasAction('aim') && !this.actionManager.hasAction('narutoRun'))
    ) {
